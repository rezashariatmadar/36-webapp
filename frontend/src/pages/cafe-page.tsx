...
import type { CafeMenuCategory, CafeOrder, CartPayload } from '../types/cafe';

type CafePageProps = {
  me: SessionPayload;
};

export default function CafePage({ me }: CafePageProps) {
  const [menu, setMenu] = useState<CafeMenuCategory[]>([]);
  ...
  const loadMenu = useCallback(async () => {
    const data = await apiFetch<{ categories?: CafeMenuCategory[] }>('/api/cafe/menu/');
  ...
  const loadOrders = useCallback(async () => {
    ...
    const data = await apiFetch<{ orders?: CafeOrder[] }>('/api/cafe/orders/');
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      setError(loadError instanceof Error ? loadError.message : 'Unable to load cafe data.');
    } finally {
      setBusy(false);
    }
  }, [loadCart, loadMenu, loadOrders]);
  ...
  const changeQuantity = async (menuItemId: number, delta: number) => {
    ...
    try {
      ...
      setError(cartError instanceof Error ? cartError.message : 'Unable to update cart.');
    }
  };
  ...
  const checkout = async () => {
    ...
    setBusy(true);
    try {
      await apiFetch<{ detail?: string }>('/api/cafe/checkout/', {
        method: 'POST',
  ...
  const reorder = async (orderId: number) => {
    setError('');
    try {
      const updatedCart = await apiFetch<CartPayload>(`/api/cafe/orders/${orderId}/reorder/`, { method: 'POST' });
      setCart(updatedCart);
            ...
            <div className="mt-3 space-y-3">
              {orders.map((order) => (
                <div key={order.id} className="spa-card p-3">
                  ...
                  <button
                    type="button"
                    className="btn btn-xs btn-outline border-white/30 text-white mt-2"
