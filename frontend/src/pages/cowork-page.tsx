...
import type { CoworkBooking, CoworkBookingPreview, CoworkZone } from '../types/cowork';

type CoworkPageProps = {
  me: SessionPayload;
};

export default function CoworkPage({ me }: CoworkPageProps) {
  const [zones, setZones] = useState<CoworkZone[]>([]);
  const [bookings, setBookings] = useState<CoworkBooking[]>([]);
  const [selectedSpace, setSelectedSpace] = useState('');
  const [bookingType, setBookingType] = useState('DAILY');
  const [startTime, setStartTime] = useState('');
  const [preview, setPreview] = useState<CoworkBookingPreview | null>(null);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState('');

  const loadSpaces = useCallback(async () => {
    const data = await apiFetch<{ zones?: CoworkZone[] }>('/api/cowork/spaces/');
    setZones(data.zones || []);
  }, []);
  ...
  const loadBookings = useCallback(async () => {
    ...
    const data = await apiFetch<{ bookings?: CoworkBooking[] }>('/api/cowork/my-bookings/');
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setError(loadError instanceof Error ? loadError.message : 'Unable to load cowork data.');
    } finally {
      setBusy(false);
    }
  }, [loadBookings, loadSpaces]);
  ...
  const runPreview = async () => {
    ...
    try {
      ...
      const data = await apiFetch<CoworkBookingPreview>(`/api/cowork/bookings/preview/?${query.toString()}`);
      setPreview(data);
    } catch (previewError) {
      setPreview(null);
  ...
  const book = async () => {
    ...
    setError('');
    try {
      await apiFetch<{ detail?: string }>('/api/cowork/bookings/', {
        method: 'POST',
        body: JSON.stringify({
          space_id: selectedSpace,
          booking_type: bookingType,
      ...
      setError(bookingError instanceof Error ? bookingError.message : 'Unable to book space.');
    } finally {
      setBusy(false);
    }
  };
              ...
              <div className="spa-grid md:grid-cols-2">
                {(zone.spaces || []).map((space) => (
                  <button
                    type="button"
                    key={space.id}
                    className={`spa-card p-4 text-right transition-colors ${
                      String(selectedSpace) === String(space.id) ? 'border-primary' : ''
                    }`}
        ...
        <div className="spa-card p-5">
          ...
          <div className="space-y-3 mt-3">
            <label className="form-control">
              ...
              <select
                className="select select-bordered bg-white/5 border-white/20 text-white"
                value={bookingType}
