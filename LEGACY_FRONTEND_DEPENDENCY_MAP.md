...
| HTMX | CDN script | decommissioned | Previously used for legacy cafe/cowork partial updates and polling | `theme/templates/base.html`, `theme/templates/cafe/*`, `theme/templates/cowork/*`, `cafe/views.py`, `cowork/views.py` | Completed in this increment. |
...
- Progress note: HTMX runtime include and `hx-headers` have been removed from base runtime after decommissioning remaining legacy HTMX flows.
- Progress note: server-side HTMX partial rendering branches were removed from `cafe/views.py` and `cowork/views.py`.
