...
| HTMX | CDN script | active (legacy staff-only runtime) | Legacy barista dashboard polling and order mutation partial updates | `theme/templates/cafe/barista_dashboard.html`, `theme/templates/cafe/partials/order_list.html`, `cafe/views.py` | Remove after template routes are fully retired or isolated under `/legacy/*` only. |
...
- Progress note: HTMX runtime include and `hx-headers` are now further restricted to legacy routes with active `hx-*` interactions, while legacy auth pages do not load HTMX.
